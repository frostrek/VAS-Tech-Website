...
const ChatBotDemo = () => {
    ...
    return (
        <div className={`w-full rounded-2xl border overflow-hidden flex flex-col md:flex-row ${theme === 'dark' ? 'bg-dark-card border-dark-accent/20' : 'bg-white border-gray-200'}`}>
            ...
            {/* Chat Area */}
            <div className={`flex-1 flex flex-col h-[500px] border-b md:border-b-0 md:border-r ${theme === 'dark' ? 'border-dark-accent/20 bg-[#0a0a0a]' : 'border-gray-200 bg-gray-50'}`}>
                {/* Chat Header */}
                <div className={`p-4 border-b flex items-center gap-3 ${theme === 'dark' ? 'bg-dark-card border-dark-accent/20' : 'bg-white border-gray-200'}`}>
                    <div className="relative">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${theme === 'dark' ? 'bg-dark-accent/20 text-dark-accent' : 'bg-[#B07552]/10 text-[#B07552]'}`}>
                        ...
                        {messages.map((msg, idx) => (
                            ...
                            >
                                <div className={`flex items-end gap-2 max-w-[80%]`}>
                                    {msg.type === 'bot' && (
                                        <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mb-1 ${theme === 'dark' ? 'bg-dark-accent/20 text-dark-accent' : 'bg-[#B07552]/10 text-[#B07552]'}`}>
                                    ...
                                    <div className={`
                                        ...
                                        ${msg.type === 'user' 
                                            ? (theme === 'dark' ? 'bg-dark-accent text-dark-bg rounded-br-sm' : 'bg-[#B07552] text-white rounded-br-sm')
                            ...
                            <div className={`flex items-end gap-2 max-w-[80%]`}>
                                ...
                                <div className={`px-4 py-3 rounded-2xl rounded-bl-sm ${theme === 'dark' ? 'bg-dark-bg border border-dark-accent/20' : 'bg-white border border-gray-200'}`}>
                                    <div className="flex gap-1">
                                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0 }} className={`w-1.5 h-1.5 rounded-full ${theme === 'dark' ? 'bg-dark-accent/50' : 'bg-[#B07552]/50'}`} />
                                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0.2 }} className={`w-1.5 h-1.5 rounded-full ${theme === 'dark' ? 'bg-dark-accent/50' : 'bg-[#B07552]/50'}`} />
                                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0.4 }} className={`w-1.5 h-1.5 rounded-full ${theme === 'dark' ? 'bg-dark-accent/50' : 'bg-[#B07552]/50'}`} />
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                ...
                {/* Chat Input */}
                <div className={`p-4 border-t ${theme === 'dark' ? 'bg-dark-card border-dark-accent/20' : 'bg-white border-gray-200'}`}>
                    <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${theme === 'dark' ? 'bg-dark-bg border-dark-accent/20' : 'bg-gray-50 border-gray-200'}`}>
                        ...
                        <button disabled className={`p-1.5 rounded-full ${theme === 'dark' ? 'bg-dark-accent text-dark-bg opacity-50' : 'bg-[#B07552] text-white opacity-50'}`}>
                            <Send className="w-4 h-4" />
                        </button>
                    </div>
                    {step >= CHAT_SEQUENCE.length && (
                        <div className="mt-3 text-center">
                            <button onClick={handleReset} className={`text-xs font-medium ${theme === 'dark' ? 'text-dark-accent hover:text-orange-400' : 'text-[#B07552] hover:text-orange-600'}`}>
                                Restart Demo
                            </button>
                        </div>
                    )}
            ...
            {/* AI Insights Panel */}
            <div className={`w-full md:w-64 p-6 flex flex-col ${theme === 'dark' ? 'bg-dark-bg' : 'bg-gray-50'}`}>
                <h4 className={`text-xs font-bold uppercase tracking-wider mb-6 flex items-center gap-2 ${theme === 'dark' ? 'text-dark-accent' : 'text-[#B07552]'}`}>
                        ...
                        <AnimatePresence mode="popLayout">
                            {messages.filter(m => m.type === 'bot' && (m.intent || m.status)).slice(-1).map((msg, i) => (
                                <motion.div 
                                    ...
                                    className={`px-3 py-2 border rounded-lg text-sm flex items-center gap-2 ${theme === 'dark' ? 'border-dark-accent/30 bg-dark-accent/10 text-dark-accent' : 'border-[#B07552]/30 bg-[#B07552]/10 text-[#8A5A35]'}`}
                                >
